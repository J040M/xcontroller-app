...
<template>
  <div class="h-full flex flex-col bg-black">
    <div class="shrink-0 flex items-center justify-between px-4 py-2 bg-surface-container-high border-b border-outline-variant">
    ...
    <Terminal class="terminal-mobile flex-1 min-h-0" prompt="xcontroller >" />
  </div>
</template>
...
.terminal-mobile :deep(.p-terminal-prompt) {
  color: #00dce5;
  font-weight: 700;
}

.terminal-mobile :deep(.p-terminal-input) {
  color: #dce4e4;
  background: transparent;
  font-family: 'JetBrains Mono', monospace;
}

.terminal-mobile :deep(.p-terminal-command) {
  color: #00f5ff;
}

.terminal-mobile :deep(.p-terminal-response) {
  color: #b9caca;
}
</style>
